//////////////////////////////////////////////////////
// Matrix TCL Pro 2.0
// Copyright (c) 2000 Techsoft Pvt. Ltd. All Rights Reserved.
// File name: Matrix.cc - Implementation of matrix class
...
inline float epsilon (complex<float> v) 
{
   return std::numeric_limits<float>::epsilon() * 1e2;
}

inline double epsilon (complex<double> v) 
{
   return std::numeric_limits<double>::epsilon() * 1e2; 
}

inline long double epsilon (complex<long double> v) 
{
   return std::numeric_limits<long double>::epsilon() * 1e2; 
}

template <class T> inline 
T epsilon (const T& v) 
{ 
   T e = numeric_limits<T>::epsilon() * 1e2;
   return v > T(1) ? v * e : e;
}
...
template <class T> matrix<T>::base_mat* 
matrix<T>::allocator (AllocType bOpt, size_t nrow, size_t ncol)
{
   static base_mat *mlist[MAX_MATRIX_CACHE] = { NULL, NULL, NULL, NULL };
...
template <class T> void
matrix<T>::rand (unsigned RandMax, unsigned RandSeed)
{
   if (RandMax == 0)
      RandMax = 1;
   if (RandMax > RAND_MAX)
...
template <class T>
T matrix<T>::lud (valarray<size_t>& ri)
{
   size_t i,j,k;
   double ta,tb;
   ...
   for (k=0; k < n-1; k++)
   {
      ...
      if (abs( pv[kpos+k]) < epsilon( pv[kpos+k]))
         throw runtime_error( "matrix<T>::LUD: Singular matrix!");
      det *= pv[kpos+k];
...
matrix<T>::chold ()
{
   ...
   for (i=0; i < n; i++)
      for (j=i; j < n; j++)
      {
         ...
         {
            if (s  <= epsilon(s))
               throw runtime_error( "matrix<T>::chold: Matrix is not symmetric or positive definite!");
            pa[i*n+i] = sqrt( s);
         }
         else
            pa[j*n+i] = s / pa[i*n+i];
...
matrix<T>::qrbksb (const matrix<T>& r, const valarray<T>& v, valarray<T>& s) const
{
   ...
   if (m != v.size())
      throw invalid_argument( "matrix<T>::qrsolve: Inconsistent vector size!");
   if (n != s.size())
      s.resize( n);
...
matrix<T>::svd (matrix<T>& vc, valarray<T>& w)
{
   ...
   for (k=n-1; ; k--)
   {
      for (its=1; its <= 30; its++)
      {
         ...
         if (its == 30)
            throw runtime_error( "matrix<T>::svd: No convergence in 30 iterations!");
...
void matrix<T>::inv ()
{
   ...
   for (k=0; k < n; k++)
   {
      ...
      if (ta < epsilon( a))
         throw runtime_error( "matrix<T>::inv: Singular matrix!");
...
matrix<T>::hqr2 (valarray<T>& d, valarray<T>& e, matrix<T>& v, bool eivec)
{
   ...
   while (n >= low) 
   {
      ...
      else 
      {
         ...
         if (++iter > 250)
            throw runtime_error( "matrix<T>::hqr2: Too many iterations!");
...
matrix<T>::tql2 (valarray<T>& d, valarray<T>& e, bool eivec)
{
   ...
   for (l=0; l < n; l++) 
   {
      ...
      do 
      {
         ...
         {
            if (iter++ == 30) 
               throw runtime_error("matrix<T>::tql2: Too many iterations!");
...
matrix<T>::tred2 (valarray<T>& d, valarray<T>& e, bool eivec)
{
   ...
   if (n != mPtr->ncol)
      throw invalid_argument( "matrix<T>::tred2: non-square matrix!");
