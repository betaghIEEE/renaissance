...
/*
const imageType imageType::inverse()
{
    ...
    double **tempMat;
    if ( m_rows == m_cols)
        square->setA(m_rows, m_data);
        else 
        return NULL;
    if (square->computeInverse())
        {
            n = square->getInverse(tempMat);
            temp = imageType (n,n);
            for (i = 0; i < n; i++)
                for (j = 0; j < n; j++)
                    temp.setValue (i,j, tempMat[i][j]);
        }
