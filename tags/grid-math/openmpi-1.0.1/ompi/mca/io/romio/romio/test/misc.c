...
/* process 0 takes the file name as a command-line argument and 
   broadcasts it to other processes */
	...
	fprintf(stderr, "disp = \%ld, datarep = %s, should be 1000, native\n\n", disp, datarep);

    if (!mynod) fprintf(stderr, "testing MPI_File_get_byte_offset\n");
    MPI_File_get_byte_offset(fh, 10, &disp);
    if (disp != (1000+20*sizeof(int))) fprintf(stderr, "byte offset = \%ld, should be %d\n\n", disp, (int) (1000+20*sizeof(int)));
    ...
    MPI_File_get_byte_offset(fh, disp, &offset);
    if (offset != 1000)
	fprintf(stderr, "file pointer posn in bytes = \%ld, should be 1000\n\n", offset);
