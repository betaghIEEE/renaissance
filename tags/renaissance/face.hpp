...
public:
  ...
  void setVertexIds(vector<int> &ids) { m_pointId = ids; }
  void setVertexNormals(vector<Vector3D> &normals) { m_vertexNormal = normals; }
  void setPoints(vector<Point3D> &pts) { m_point = pts; }
  void setTextureCoordinates(vector<Point2D> &pts) { m_texCoord = pts; }
