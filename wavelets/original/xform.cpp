...
void xform::oddSplit ( myVector &input, myVector &result)
{
	int size = input.Size();
	int i;
	int l = size/2;
...
void xform::oddPick ( myVector &a, myVector &d , myVector &result)
{
    int i, j, k;
    int size = a.Size();
    int s2 = size /2;
...
void xform::evenPick ( myVector &a, myVector &d , myVector &result)
{
    int i, j, k;
    int size = a.Size();
    int s2 = size /2;
...
void xform::hWaveEven (myVector &input, myVector &result)
{
	hWaveEven(input, result, 1);
}
...
void xform::hWaveEven (myVector &input, myVector &result, int resolution)
{
	myVector S;
	myVector Ra, Rd, R, F;
	haarGen haarD;
...
void xform::hWaveInverseO (myVector &input, myVector &result)
/*
...
//cout << "First value " << result[j] << endl;
        result[j] *= sqhalf;
	//if ( (result[j] < 0.0001) && (result[j] > -0.00001))
		//result[j] = 0.0;
	...
	for ( i = 0; i < l2 + 1; i++)
	{
		j = 2 * i;
		m = l2 + i;
		result[j] = sqhalf * (input[i] - input[m]);
		if ( (result[j] < 0.0001) && (result[j] > -0.00001))
			result[j] = 0.0;
		//cout << "\t" << j ;
	...
	//cout << endl;
	//result[1] = (input[1] - input[l2 + 1])/sqhalf;
	//result[2] = sqhalf * (input[1] - input[l2]);
	//result[l-1] = sqhalf * (input[l2-1] - input[l -1]);
	
}

void xform::hWaveInverseO (myVector &input, myVector &result, int resolution)
/*
...
void xform::hWaveInverseE (myVector &input, myVector &result)
/*
	Haar Wavelet Inverse:
	Purpose: Return result as the inverse transform of a simple Haar Wavelet 			Transform.  The general method to the madness is as follows:
		for all i element of [0,l]  where l is the length of the source
...
void xform::hWaveInverseE (myVector &input, myVector &result, int resolution)
/*
	Haar Wavelet Inverse - Multi-Resolution
	Purpose: To separate a vector into segments to transformed, and 
		used in the transform of the whole.  Note this version is based
